  ...
  mounted() {
    ...
    BimAir.Loader({ isShareArrayBuffer: true }).then(() => {
      ...
      viewer.loadModels([id, model1], true).then((lightModels) => {
        ...
        // get views
        let views = viewer.panelManager.viewPointPanel.getViewPoints();
        console.log(views);
        // add a view
        viewer.panelManager.viewPointPanel.addViewPoint("new");
        // delete a view with the index
        viewer.panelManager.viewPointPanel.getViewPoints().then(() => {
          viewer.panelManager.viewPointPanel.deleteViewPoint(1);
          viewer.panelManager.viewPointPanel.restoreViewPoint(1);
        });
