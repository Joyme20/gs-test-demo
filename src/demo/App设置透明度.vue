...
export default {
  ...
  mounted() {
    ...
    BimAir.Loader({ isShareArrayBuffer: true }).then(() => {
      ...
      let obj1 = "QMD+M+4G+HYD+XDRA01+EL002";
      let obj_zg8 = "QMD+M+4G+HYD+XDRA01+EL008";

      let oo = "6305deacb15c8447499035fe";
      ...
      viewer.loadModels([id, model1], true).then((lightModels) => {
        ...
        let children = lightModels[1].treeNodeObject.childNodes;
        let child = children[0].childNodes;

        let segO1 = child[3].segmentObject;
        console.log(segO1);

        // let sub = segO1.segment.subsegments;
        // console.log(sub, 'sub');

        let material = new BimAir.Material(viewer, "test");
        material.setTransparency(0.6);
        // segO1.setMaterial(material);
        // viewer.updateDisplay();

        let treeNode = viewer.structureTreeManager.getTreeNodesByName("MED-C-01-CF-ZCF-002-001(2#机组)");
        console.log(treeNode);
        let seg = treeNode.items[0].segmentObject;
